...
//solucionado error por el que aceptaba un usuario o una contrasegna que fuesen de tamaño menor que la real pero coincidiese en todos los caracteres
//(por ejemplo escribir Jef se aceptaba cuando el usuario era Jefe)
...
void main() {
	...
	int s = 1, i = 0, j, k, a, b;
	struct user usuario[TAM_2] = { {"Jefe","1234"}, {"empleado1","1234"}, {"empleado2","qwe123qwe"}, {"empleado3","yo12"} };
	...
	do {
		...
		for (k = 0; k < TAM_2; k++) {
			...
			while (temp.usuario[j] != '\0') {
				if (temp.usuario[j] != usuario[k].usuario[j]) {
					a++;
					break;
				}
				j++;
			}
			if ((usuario[k].usuario[j] != '\0') && (a == k)) {
				a++;
			}
			j = 0;
			if (a == k) {
				while (temp.contrasegna[j] != '\0') {
					if (temp.contrasegna[j] != usuario[k].contrasegna[j]) {
						b++;
						break;
					}
					j++;
				}
				if ((usuario[k].contrasegna[j] != '\0') && (b == 0)) {
					b++;
				}
			}
			if (b != 0) {
